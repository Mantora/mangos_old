...
void WorldSession::HandleCharFactionOrRaceChangeOpcode(WorldPacket& recv_data)
{
    ...
    QueryResult *result = CharacterDatabase.PQuery("SELECT at_login FROM characters WHERE guid ='%u'", guid);
    if (!result)
    {
    ...
    CharacterDatabase.BeginTransaction();
    CharacterDatabase.PExecute("UPDATE characters set name = '%s', race = '%u', at_login = at_login & ~ %u WHERE guid ='%u'", newname.c_str(), race, uint32(used_loginFlag), guid);
    ...
    {
        // Delete all Flypaths
        CharacterDatabase.PExecute("UPDATE characters set taxi_path = '' WHERE guid ='%u'", guid);
        // Delete all current quests
        CharacterDatabase.PExecute("DELETE FROM `character_queststatus` WHERE `status` = 3 AND guid ='%u'", guid);
        // Reset guild
        CharacterDatabase.PExecute("DELETE FROM `guild_member` WHERE `guid`= '%u'", guid);
        // Delete Friend List
        CharacterDatabase.PExecute("DELETE FROM `character_social` WHERE `guid`= '%u'", guid);
        CharacterDatabase.PExecute("DELETE FROM `character_social` WHERE `friend`= '%u'", guid);
        // Leave Arena Teams
        Player::LeaveAllArenaTeams(guid);
        // Reset Language (will be added automatically after faction change)
        CharacterDatabase.PExecute("DELETE FROM `character_spell` WHERE `spell` IN (668, 7340, 671, 672, 814, 29932, 17737, 816, 7341, 669, 813, 670) AND guid ='%u'", guid);
        ...
        if(team == BG_TEAM_ALLIANCE)
            CharacterDatabase.PExecute("INSERT INTO `character_homebind` VALUES ('%u','0','1519','-8867.68','673.373','97.9034')", guid);
        else
            CharacterDatabase.PExecute("INSERT INTO `character_homebind` VALUES ('%u','1','1637','1633.33','-4439.11','15.7588')", guid);
        ...
        if(QueryResult *result2 = WorldDatabase.Query("SELECT alliance_id, horde_id FROM player_factionchange_reputations"))
        {
            do
            {
                ...
                CharacterDatabase.PExecute("DELETE FROM character_reputation WHERE faction = '%u' AND guid = '%u'",team == BG_TEAM_ALLIANCE ? reputation_horde : reputation_alliance, guid);
    ...
    std::string IP_str = GetRemoteAddress();
    sLog.outChar("Account: %d (IP: %s), Character guid: %u Change Race/Faction to: %s", GetAccountId(), IP_str.c_str(), guid, newname.c_str());
