...
void WorldSession::SendGMTicketGetTicket(uint32 status, GMTicket *ticket /*= NULL*/, bool isAssigned)
{
...
void WorldSession::HandleGMTicketGetTicketOpcode( WorldPacket & /*recv_data*/ )
{
    ...
    if(ticket)
    {
        if(ticket->HasResponse())
            ...
            SendGMTicketGetTicket(0x06, ticket, false);
    }
    else
...
void WorldSession::HandleGMTicketCreateOpcode( WorldPacket & recv_data )
{
    ...
    {
        if(itr->second->GetSession()->GetSecurity() >= SEC_MODERATOR && itr->second->isAcceptTickets())
