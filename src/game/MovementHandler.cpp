...
void WorldSession::HandleMoveNotActiveMoverOpcode(WorldPacket &recv_data)
{
    ...
    {
        sLog.outDebug("HandleMoveNotActiveMover: incorrect mover guid: mover is %s and should be %s instead of %s",
...
void WorldSession::HandleMoverRelocation(MovementInfo& movementInfo, Unit* mover, Player* plMover)
{
    ...
    if (plMover)
    {
        if (movementInfo.HasMovementFlag(MOVEFLAG_ONTRANSPORT) && !plMover->GetTransport())
        {
            /* process anticheat check */
            if (GetPlayer()->GetAntiCheat()->CheckNeeded(plMover))
