...
void OPvPCapturePoint::AddGO(uint32 type, uint32 guid, uint32 entry)
{
    ...
    m_Objects[type] = ObjectGuid(HIGHGUID_GAMEOBJECT, guid);
...
void OPvPCapturePoint::AddCre(uint32 type, uint32 guid, uint32 entry)
{
    ...
    m_Creatures[type] = ObjectGuid(HIGHGUID_UNIT, guid);
...
{
	ObjectGuid ObGuid = m_Creatures[type];
    if(ObGuid.IsEmpty())
    {
...
{
	ObjectGuid ObGuid = m_Objects[type];
    if(ObGuid.IsEmpty())
        return false;
