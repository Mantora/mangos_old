...
{
    return isAlive() && !HasInvisibilityAura() && !HasStealthAura() && (IsPvP() || sWorld.IsPvPRealm())  && !HasMovementFlag(MOVEFLAG_FLYING) && !IsFlying();
}
    ...
    if(saBounds.first != saBounds.second)
    {
        ...
        for(SpellAreaForAreaMap::const_iterator itr = saBounds.first; itr != saBounds.second; ++itr)
            if(itr->second->autocast && itr->second->IsFitToRequirements(this,zone,area))
				...
				if(itr->second->spellId != 58600 || (HasAuraType(SPELL_AURA_MOD_INCREASE_SPEED) || HasAuraType(SPELL_AURA_FLY) || IsFlying()))
...
void Player::_SaveInventory()
{
    ...
    if (error)
    {
        ...
        sWorld.BanAccount(BAN_CHARACTER, GetName(), 0, "WPE/Dupe", "auto-ban");
        return;
    }
